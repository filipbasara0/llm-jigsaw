...
*Results averaged across 20 images per model per grid size. Claude Opus 4.5 tested only on 3×3 (20% solve rate, 47.2% accuracy). Gemini-3.0 Pro and GPT-5.2 were using low reasoning effort, while Opus 4.5 was using high reasoning effort.*
...
4. **Accuracy degrades more gracefully**: Partial accuracy remains reasonable, although frontier models hit a wall between 50% and 80% for 4x4 and 5x5 puzzles

## Detailed Results by Grid Size

### 3×3 Grid (9 pieces)

| Model | Accuracy | Solve Rate | Avg Turns | Avg Tokens |
|-------|----------|------------|-----------|------------|
...
- **Reasoning effort**: GPT-5.2 and Gemini 3 Pro used `low` reasoning effort; Claude Opus 4.5 used `high` reasoning effort

> **Note on reasoning effort**: Informal testing with `high` reasoning effort for GPT-5.2 and Gemini 3 Pro showed slightly better performance, but at significantly higher cost (a single puzzle could consume ~1M tokens with Gemini) and much longer solving times. We opted for `low` reasoning to keep the benchmark practical.
