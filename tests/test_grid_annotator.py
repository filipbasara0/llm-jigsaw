...
class TestGridAnnotator:
    ...
    def test_init(self):
        ...
        assert annotator.grid_rows == 8
        assert annotator.grid_cols == 8

    def test_init_rectangular(self):
        """Test initialization with rectangular grid."""
        annotator = GridAnnotator(grid_size=(3, 5))

        assert annotator.grid_rows == 3
        assert annotator.grid_cols == 5
    ...
    def test_coordinate_format_description(self):
        ...
        assert "1,1" in desc
        assert f"{annotator.grid_rows},{annotator.grid_cols}" in desc
        assert "row" in desc.lower()
        assert "col" in desc.lower()

    def test_coordinate_format_description_rectangular(self):
        """Test coordinate format description for rectangular grid."""
        annotator = GridAnnotator(grid_size=(3, 5))

        desc = annotator.get_coordinate_format_description()

        assert "1,1" in desc
        assert "3,5" in desc  # rows,cols
