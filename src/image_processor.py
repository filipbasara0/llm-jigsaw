...
class ImageProcessor:
    ...
    def __init__(
        ...
        Args:
            ...
            grid_size: Size of the grid. Can be:
                - int: Square grid (e.g., 4 for 4x4)
                - tuple[int, int]: Rectangular grid as (rows, cols) (e.g., (3, 5) for 3x5)
        ...
        # Parse grid_size into rows and cols
        ...
        self._current_arrangement: list[list[int]] = [
            [row * self.grid_cols + col for col in range(self.grid_cols)]
    ...
    def _load_and_prepare_image(self) -> np.ndarray:
        ...
        # Crop to make dimensions divisible by grid dimensions
        h, w = img_array.shape[:2]
        new_h = (h // self.grid_rows) * self.grid_rows
        new_w = (w // self.grid_cols) * self.grid_cols
    ...
    def _calculate_piece_dimensions(self) -> tuple[int, int]:
        ...
        return h // self.grid_rows, w // self.grid_cols
    ...
    def shuffle(self) -> None:
        ...
        for r in range(self.grid_rows):
            for c in range(self.grid_cols):
                self._current_arrangement[r][c] = flat[r * self.grid_cols + c]
    ...
    def _parse_coordinate(self, coord: str) -> tuple[int, int]:
        ...
        try:
            ...
            if not (0 <= row < self.grid_rows and 0 <= col < self.grid_cols):
                raise ValueError(
                    f"Coordinate {coord} out of bounds for grid size {self.grid_rows}x{self.grid_cols}"
                )
    ...
    def apply_swap(self, coord_a: str, coord_b: str) -> bool:
        ...
        try:
            ...
            self._current_arrangement[row_a][col_a], self._current_arrangement[row_b][col_b] = (
                self._current_arrangement[row_b][col_b],
                self._current_arrangement[row_a][col_a],
            )
    ...
    def get_current_state(self) -> np.ndarray:
        ...
        """
        h = self._piece_height * self.grid_rows
        w = self._piece_width * self.grid_cols
    ...
    def get_solution_mapping(self) -> dict[str, str]:
        ...
        for row in range(self.grid_rows):
            for col in range(self.grid_cols):
                piece_idx = self._current_arrangement[row][col]
                correct_row = piece_idx // self.grid_cols
                correct_col = piece_idx % self.grid_cols
    ...
    def count_correct_pieces(self) -> int:
        ...
        for row in range(self.grid_rows):
            for col in range(self.grid_cols):
                piece_idx = self._current_arrangement[row][col]
                expected_idx = row * self.grid_cols + col
    ...
    def is_solved(self) -> bool:
        ...
        return self.count_correct_pieces() == self.total_pieces
    ...
    def copy(self) -> "ImageProcessor":
        ...
        new_processor.grid_rows = self.grid_rows
        new_processor.grid_cols = self.grid_cols
    ...
    def grid_size(self) -> tuple[int, int]:
        """Get grid dimensions as (rows, cols) tuple."""
        return (self.grid_rows, self.grid_cols)
