...
class PuzzleGame:
    ...
    def step(self) -> TurnResult:
        ...
        if self.config.verbose:
            logger.info(f"Turn {self.turn_number}: {correct_before}/{self.processor.total_pieces} correct")
    ...
    def run(self) -> GameResult:
        ...
        if self.config.save_intermediate_images and self.output_dir:
            self._save_image(self._get_annotated_image(), "initial_state.png")
        ...
        if self.config.verbose:
            ...
            logger.info(f"Final: {result.final_correct}/{result.total_pieces} ({result.accuracy:.1f}%)")
            logger.info(f"Max achieved: {result.max_correct_achieved}/{result.total_pieces} ({result.max_accuracy:.1f}%)")
