...
class GridAnnotator:
    ...
    def __init__(self, grid_size: int | tuple[int, int]):
        """
        Initialize the grid annotator.

        Args:
            grid_size: Size of the grid. Can be:
                - int: Square grid (e.g., 8 for 8x8)
    ...
    def _generate_distinct_colors(self) -> list[tuple[int, int, int]]:
        ...
        for i in range(self.grid_rows):
            for j in range(self.grid_cols):
                ...
                hue = ((i * self.grid_cols + j) * 0.618033988749895) % 1.0
                saturation = 0.7
                value = 0.9
    ...
    def annotate(
        ...
        """
        h, w = image.shape[:2]
        piece_h = h // self.grid_rows
        piece_w = w // self.grid_cols
        ...
        if mode in ["cell_labels", "both"]:
            self._draw_cell_labels(draw, margin, piece_h, piece_w, label_color, label_bg_color)
    ...
    def _draw_colored_borders(
        self, draw: ImageDraw.ImageDraw, margin: int, piece_h: int, piece_w: int, border_width: int
    ) -> None:
        ...
        for row in range(self.grid_rows):
            for col in range(self.grid_cols):
                color_idx = row * self.grid_cols + col
    ...
    def _draw_border_labels(
        self, draw: ImageDraw.ImageDraw, margin: int, piece_h: int, piece_w: int
    ) -> None:
        ...
        for col in range(self.grid_cols):
            ...
            draw.text((x - text_w // 2, y - text_h // 2), label, fill=text_color, font=font)
    ...
    def _draw_cell_labels(
        ...
        for row in range(self.grid_rows):
            for col in range(self.grid_cols):
                ...
                # Draw text
                draw.text((x + padding, y + padding), label, fill=text_color, font=font)
    ...
    def get_coordinate_format_description(self) -> str:
        ...
        return (
            ...
            f'Top-left is "1,1", bottom-right is "{self.grid_rows},{self.grid_cols}". '
